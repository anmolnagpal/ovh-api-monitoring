...
<div class="container-fluid">
    ...
    foreach ($project as $p) {
        ?>
        <h2 class="mt-3">Project &laquo; <?= $p->description ?> &raquo;<br>
            ...
            <?php foreach ($p->instances as $index => $i) { ?>
                ...
                <tr data-project="<?= $p->project_id ?>" data-instance="<?= $i->id ?>" class="<?= ($index % 2 === 0 ? 'even' : 'odd') ?>">
                    ...
                    <?php if (count($instanceVolumes) === 0) { ?>
                        <td colspan="5"></td>
                    <?php } else { ?>
                        <th><?= $instanceVolumes[0]->name ?><br>
                            <small><?= $instanceVolumes[0]->id ?></small>
                        </th>
                ...
                if (count($instanceVolumes) > 1) {
                    for ($k = 1; $k < count($instanceVolumes); $k++) {
                        ...
                        <tr class="<?= ($index % 2 === 0 ? 'even' : 'odd') ?>">
                            <th><?= $instanceVolumes[$k]->name ?><br>
                                <small><?= $instanceVolumes[$k]->id ?></small>
                            </th>
